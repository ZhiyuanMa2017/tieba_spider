...
def get_content2(url, name):
    ...
    for ul in liTags:
        ...
        try:
            ...
            # 获取回复者的贴吧链接
            content['reply_author_link'] = "http://tieba.baidu.com/" + \
                                           ul.find(attrs={"class": 'p_author_name j_user_card'})["href"]
...
def get_content(url):
    ...
    for li in liTags:
        ...
        try:
            ...
            print(li.find('span', attrs={'class': 'threadlist_rep_num center_text'}).text.strip())
            content['creatTime'] = li.find('span', attrs={"class": 'pull-right is_show_create_time'}).text.strip()
            print (li.find( 'span', attrs={'class': 'pull-right is_show_create_time'}).text.strip())
...
#                                charset='utf8mb4')
#         for c in content:
#             sql = "insert into crawler values(NULL,'" + c['title'] + "','" + c['link'] + "','" + c['author'] + "','" + \
#                   c['creatTime'] + "','" + c['responseNum'] + "','" + c['text'] + "')"
#             for b in c['reply']:
#                 try:
#                     sql2 = "insert into reply values(NULL,'" + b['id_link'] + "','" + b['id_name'] + "','" + b[
#                         'reply_author'] + "','" + b['reply_author_link'] + "','" + b['reply_text'] + "')"
#                     cursor2 = conn.cursor()
...
def writeTxt(content):
    ...
    for c in content:
        f.write('{} \t {} \t {} \t {} \t {} \n'.format(c['title'], c['link'], c['author'], c['creatTime'], c['responseNum']))
        # print(c['title'])


url = "https://tieba.baidu.com/f?kw=背锅&ie=utf-8"
page = 170
